...
class HuiController(tk.Tk):
     def __init__(self, *args, **kwargs):
        ...
        self.frames['manual'].grid(row=4,column=3, sticky="nsew")
...
class manual(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button1.pack(side= "right", fill= "x", pady=5)
        button2 = tk.Button(self, text= "Cancel", font= ("Copper Black", 20), fg= "green",
        command= lambda: controller.give_mm_send_bool(False))
        button2.pack(side= "right", fill= "x", pady=5)

class start(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent, bg= "#d459de")
        self.controller = controller
        label = tk.Label (self, text= "Welcome to PAPI, Please Scan QR", width= 10, height= 8, font= ("Comic Sans Ms", 50), bg= "#d459de")    
